...
def stream_start(request, stream_id):
    """Start a stream with rate limiting"""
    stream = get_object_or_404(Stream, id=stream_id, user=request.user)

    if stream.status == 'running':
        messages.warning(request, 'Stream already running')
        return redirect('stream_detail', stream_id=stream.id)
    ...
    try:
        ...
        if not broadcast_id:
            raise Exception("Broadcast creation failed")

        # Start FFmpeg
        process_id = manager.start_ffmpeg_stream()
        if not process_id:
            raise Exception("FFmpeg start failed")
        ...
        messages.success(request, 'Stream started!')
        ...
        messages.error(request, 'Failed to start stream')
